...
def buyorder():
    ...
    target_words = ["SELL", "BUY" ,"100%", "(1)","(2)","(3)"]
    ...
    for i, (word, x, y) in enumerate(converted_data):
        if word == "BUY":
            converted_data[i] = ("BUYMAIN", x, y)
              # Stop after renaming the first occurrence
            break

    print(converted_data)
    # Sample data
    buy_y = next(y for word, _, y in converted_data if word == "BUYMAIN")
    # Extract x from the first BUY
    buy_x = next(x for word, x, _ in converted_data if word == "BUYMAIN")
    ref=("ref", buy_x,buy_y-220)
    converted_data.append(ref)
    #for queue5 cordinates in
    fin_y = next(y for word, _, y in converted_data if word == "ref")
    #equation of a line
    fin_x = (buy_y+307/0.65)
    price_chart=("price_chart",fin_x,fin_y)
    converted_data.append(price_chart)
        # Define sorting priority
    priority = {"price_chart":1,"BUYSIDE":2,"100%":3,"BUYMAIN":4,"SELL":5}
    ...
    for word, x, y in sorted_data:
        if word == "price_chart":
            pyautogui.moveTo(x,buy_y,duration=10)
           # pyautogui.click()
        else:
    ...
    with open("sellside.txt","r") as file:
        ...
        for word, x, y in sorted_data:
            if word == "price_chart":
                ...
                pyautogui.moveTo(int(x), int(y), duration=10)
                print(f"Moved cursor to '{word}' at ({x}, {y})")
                #pyautogui.click()
